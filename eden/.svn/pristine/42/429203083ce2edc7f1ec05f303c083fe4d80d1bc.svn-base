...
require_once dirname(__FILE__).'/google/imap.php';
require_once dirname(__FILE__).'/google/smtp.php';
...
class Eden_Google extends Eden_Class {
	...
	/**
	 * Returns google mail methods
	 *
	 * @param *string 
	 * @return Eden_Google_Imap
	 */
	public function imap($user, $token) {
		//argument test
		Eden_Google_Error::i()
			->argument(1, 'string')
			->argument(2, 'string');
		
		return Eden_Google_Imap::i($user, $token);
	}
	 ...
	 *
	 * @param *string 
	 * @return Eden_Google_Smtp
	 */
	public function smtp($user, $token) {
		//argument test
		Eden_Google_Error::i()
			->argument(1, 'string')
			->argument(2, 'string');
		
		return Eden_Google_Smtp::i($user, $token);
	}
